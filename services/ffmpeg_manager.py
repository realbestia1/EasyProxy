    ...
    async def get_stream(self, url: str, headers: dict = None, clearkey: str = None) -> Optional[str]:
        ...
        if stream_id in self.processes:
            ...
            if proc.returncode is None:
                ...
                # Wait for it.
                logger.info("Stream %s is initializing. Waiting for playlist...", stream_id)
                for _ in range(100):  # Wait up to 10s
                    if os.path.exists(playlist_path):
                        return f"{stream_id}/index.m3u8"
                    if proc.returncode is not None:
                        # Process died while waiting
                        logger.warning("Process %s died while waiting.", stream_id)
                ...
                if proc.returncode is None and not os.path.exists(playlist_path):
                    logger.warning("Stream %s timed out initializing. Restarting.", stream_id)
                    try:
                        proc.kill()
                    except Exception:
                        pass
                ...
                logger.warning("FFmpeg process for %s exitted with %s. Restarting.", stream_id, proc.returncode)
                del self.processes[stream_id]
    ...
    async def _start_ffmpeg(self, url: str, headers: dict, stream_id: str, clearkey: str = None) -> str:
        ...
        if os.path.exists(stream_dir):
            try:
                ...
                logger.error("Error cleaning stream dir %s: %s", stream_dir, e)
        ...
        headers_str = ""
        if headers:
            valid_headers = {k: v for k, v in headers.items() if
                             k.lower() not in ['host', 'connection', 'accept-encoding']}
        ...
        if clearkey:
            try:
                ...
                if keys_to_use:
                    logger.debug("Added %d decryption key(s) to FFmpeg command", len(keys_to_use))
            except Exception as e:
                logger.error("Error parsing clearkey: %s", e)
        ...
        logger.info("Starting FFmpeg for %s", stream_id)
        logger.debug("FFmpeg ClearKey for %s: %s", stream_id, clearkey)
        logger.debug("FFmpeg command for %s: %s", stream_id, cmd)
        ...
        try:
            ...
            for _ in range(300):  # Wait up to 30 seconds
                ...
                if process.returncode is not None:
                    ...
                    log_file.close()
                    logger.error("FFmpeg process died. Stderr: %s", stderr.decode()[:500])
                    return None
                await asyncio.sleep(0.1)
            else:
                log_file.close()
            ...
            try:
                ...
                for stream_id, last_access in list(self.access_times.items()):
                    ...
                    if now - last_access > 120:
                        logger.info("Stream %s idle for 120s. Terminating.", stream_id)
    ...
    async def _stop_stream(self, stream_id: str):
        if stream_id in self.processes:
            ...
            try:
                ...
                logger.error("Error killing process %s: %s", stream_id, e)
            del self.processes[stream_id]
        ...
        if os.path.exists(stream_dir):
            try:
                ...
                logger.error("Error removing stream dir %s: %s", stream_dir, e)
